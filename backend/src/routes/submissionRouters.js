...
router.get("/:id", checkLogin, SubmissionController.getSubmissionById);
...
router.post("/", checkLogin, SubmissionController.startExam);

// POST /api/submissions/:id/submit - Nộp bài (cập nhật thoiGianNop, tính điểm)
router.post("/:id/submit", checkLogin, SubmissionController.submitExam);
...
router.post("/:id/sync-answers", checkLogin, SubmissionController.answerQuestion);
